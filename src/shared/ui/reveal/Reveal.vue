...
function setInitialStyles(el: HTMLElement) {
  ...
  const dx = props.side === 'left' ? -props.distance : props.side === 'right' ? props.distance : 0
  const dy = props.side === 'up' ? -props.distance : props.side === 'down' ? props.distance : 0
  ...
  if (props.stagger) {
    ...
    children.forEach((child, i) => {
      child.style.setProperty('--rv-delay', `${props.delay + i * props.staggerStep}ms`)

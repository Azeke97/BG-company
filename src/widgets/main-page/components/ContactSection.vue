...
<template>
  <section :class="s.section">
    <div :class="s.container">
      ...
      <p :class="s.subtitle">{{ t("contact.subtitle") }}</p>

      <form v-if="!isSent" :class="s.form" @submit.prevent="submitContact">
        <input
        ...
        <i18n-t keypath="contact.consent" tag="p" :class="s.consent">
          <template #policy>
            <a href="#" target="_blank" rel="noopener" :class="s.consentLink">
              {{ t("contact.policy") }}
            </a>
          </template>
        </i18n-t>
        ...
        <button
          :class="[s.btn, s.btnPrimary, s.submitBtn]"
          type="submit"
          :disabled="isSending"
        >
